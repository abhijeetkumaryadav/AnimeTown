...
// ============================================================
// TYPES – all IDs are strings (Firestore)
// ============================================================
interface Anime {
  id: string;
  title: string;
...
// ============================================================
// CACHE HELPERS  (includes newlyAddedIds)
// ============================================================
...
// 🔙 ORIGINAL HERO SECTION (the one you had before)
function HeroSection({ anime, onPrev, onNext, onWatch, onToggleList, isInList }: any) {
  return (
    <section className="relative w-full rounded-2xl overflow-hidden bg-[#0c0d19] border border-zinc-900/60 shadow-2xl min-h-[360px] md:min-h-[440px] flex items-center">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full md:w-1/2 opacity-40 md:opacity-100 z-0">
        <img src={getSafeImage(anime?.image)} alt="Featured" className="w-full h-full object-cover object-right transition-all duration-700" />
      </div>
      <div className="absolute right-6 top-6 z-20 hidden md:flex items-center gap-1.5">
        <button onClick={onPrev} className="w-7 h-7 bg-black/40 border border-zinc-800 rounded flex items-center justify-center text-zinc-400 hover:text-red-400 transition-all"><ChevronLeft className="w-4 h-4" /></button>
        <button onClick={onNext} className="w-7 h-7 bg-black/40 border border-zinc-800 rounded flex items-center justify-center text-zinc-400 hover:text-red-400 transition-all"><ChevronRight className="w-4 h-4" /></button>
      </div>
      <div className="relative pl-6 md:pl-16 pr-6 max-w-xl z-20 space-y-4">
        <span className="inline-block bg-red-600/15 border border-red-600/30 text-red-600 text-[10px] font-bold px-2.5 py-0.5 rounded-md tracking-wider shadow-[0_0_10px_rgba(239,68,68,0.2)]">#1 Spotlight</span>
        <h1 className="text-3xl sm:text-5xl font-black tracking-tight leading-tight text-white drop-shadow">{anime?.title || "Monogatari Series"}</h1>
        <p className="hidden md:block text-xs sm:text-sm text-zinc-400 leading-relaxed max-w-md">{anime?.description || "Experience the mind‑bending supernatural world of Monogatari."}</p>
        <div className="flex items-center gap-2 pt-1">
          <button onClick={onWatch} className="flex items-center gap-1.5 bg-red-600 hover:bg-red-700 transition-all text-white text-xs font-bold py-2.5 px-5 rounded-lg shadow-lg shadow-red-600/20"><Play className="w-3.5 h-3.5 fill-current" /> Watch Now</button>
          <button onClick={onToggleList} className="p-2.5 bg-zinc-900/60 border border-zinc-800/80 rounded-lg text-white hover:bg-zinc-800 hover:border-red-500/50 transition-all">
            {isInList ? (
              <Bookmark className="w-5 h-5 text-yellow-400 fill-yellow-400 drop-shadow-[0_0_8px_rgba(234,179,8,0.6)]" />
            ) : (
      ...
      {genres.map((g: any, i: number) => (
        <button
          ...
          className={`px-3.5 py-1.5 border rounded-md text-xs font-medium whitespace-nowrap transition-all duration-200 flex items-center gap-1.5 ${
            activeGenre === g.name ? "bg-red-600 border-red-600 text-white shadow-[0_0_10px_rgba(239,68,68,0.3)]" : "bg-[#0b0c14] border-zinc-900/80 hover:border-red-500/50 text-zinc-400 hover:text-red-400"
          }`}
        >
...
// AnimeCard – original behaviour, plus optional glass badge for Newly Added
function AnimeCard({ anime, onPlay, onToggleList, isInList, rank }: any) {
  return (
    <div className="group space-y-1.5 flex-shrink-0 w-[120px] md:w-[150px]">
      <div className="relative aspect-[3/4.2] bg-zinc-900 rounded-lg overflow-hidden border border-zinc-900 group-hover:border-red-500/30 transition-all cursor-pointer" onClick={onPlay}>
        ...
        {rank ? (
          // Newly Added style: semi‑transparent glass, large #
          ...
          // Original small red "TOP" badge
          <span className="absolute top-1 left-1 bg-red-600 text-[8px] font-black px-1 rounded text-white">TOP</span>
        )}
        <button onClick={(e) => { e.stopPropagation(); onToggleList(); }} className="absolute top-1 right-1 p-1 bg-black/60 rounded-full text-white/70 hover:text-yellow-400 transition-colors z-10">
          <Bookmark className={`w-3 h-3 ${isInList ? 'fill-yellow-400 text-yellow-400 drop-shadow-[0_0_6px_rgba(234,179,8,0.5)]' : ''}`} />
        </button>
      ...
      <h4 className="text-[11px] font-bold text-zinc-200 truncate group-hover:text-red-400 cursor-pointer">{anime.title}</h4>
...
function EpisodeCard({ episode, anime, onPlay }: any) {
  return (
    <div className="group space-y-1.5 flex-shrink-0 w-[120px] md:w-[150px]">
      <div className="relative aspect-[3/4.2] bg-zinc-900 border border-zinc-900 rounded-lg overflow-hidden group-hover:border-red-500/30 transition-all cursor-pointer" onClick={onPlay}>
      ...
      <h4 className="text-[11px] font-bold text-zinc-200 truncate group-hover:text-red-400 cursor-pointer">{anime?.title || "Unknown"}</h4>
...
function SectionHeader({ title, icon, onViewAll }: any) {
  return (
    <div className="flex justify-between items-center">
      ...
      {onViewAll && (
        <button onClick={onViewAll} className="text-[11px] font-semibold text-red-600 hover:text-red-400">View all</button>
      )}
    </div>
  );
}
...
export default function HomePage() {
  ...
  const [newlyAddedIds, setNewlyAddedIds] = useState<string[]>([]);   // NEW
  ...
  // ---- Instant cache load (fast) – now includes newlyAddedIds ----
  useLayoutEffect(() => {
    ...
    if (cachedHome) {
      ...
      setNewlyAddedIds(cachedHome.newlyAddedIds || []);   // ← instant display
      setLoading(false);
    }
  ...
  // ---- Background API fetch for all data (including newly‑added) ----
  useEffect(() => {
      ...
      try {
        const [animeRes, episodesRes, scheduleRes, newsRes, featuredRes, newlyAddedRes] =
          await Promise.all([
            ...
            fetch('/api/newly-added').then(r => r.json()),   // ← added
          ]);
        ...
        const freshNewlyAdded = newlyAddedRes.newlyAdded || [];   // ← new
        ...
        setNewlyAddedIds(freshNewlyAdded);                     // ← set state
        ...
        saveHomeCache({
          ...
          newlyAddedIds: freshNewlyAdded,                     // ← save in cache
        });
      } catch (error) {
  ...
  // ---- Fetch user data from Supabase (initial load) ----
  useEffect(() => {
    if (!user) {
      setRawWatchHistory([]);
      setWatchlistIds([]);
  ...
  // ---- Derived data – include progress from watch history ----
  ...
  // ---- Auto-rotate banner (original 5 seconds) ----
  useEffect(() => {
  ...
  // ⭐ Newly Added – respects admin order first, then falls back to creation date
  ...
  // ---- Top Rated – minimum score 7, descending ----
  ...
  // ✅ FIXED: use animeList (full list) for featured, not language‑filtered displayAnime
    ...
    <div className="max-w-[1400px] mx-auto w-full px-4 md:px-6 py-5 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
        <div className="w-full lg:w-[70%] flex flex-col gap-6">
          ...
          {/* 🆕 Newly Added (instant from cache) */}
          <section className="space-y-3 bg-[#0a0b12] border border-zinc-900/60 rounded-xl p-4">
            <SectionHeader
              title="Newly Added"
              icon="🆕"
            ...
            <div className="flex gap-3 overflow-x-auto scrollbar-none pb-2">
              {newlyAdded.slice(0, 6).map((anime, index) => (
                <AnimeCard
                  key={anime.id}
                  anime={anime}
                  rank={index + 1}   // 👈 glass #1, #2…
                  ...
                  {["All", ...uniqueTypes].map(f => (
                    <button
                      ...
                      className={`text-[10px] font-bold px-2.5 py-1 rounded transition-all duration-200 ${
                        popularTypeFilter === f ? "bg-red-600 text-white shadow-[0_0_6px_rgba(239,68,68,0.3)]" : "text-zinc-500 hover:text-red-400"
                      }`}
                    >
                      {f}
                    </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-[#0a0b12] border border-zinc-900/60 rounded-xl p-4 space-y-3">
              ...
              <div className="flex flex-col gap-2">
                {mostWatched.map((anime, idx) => (
                  <div key={anime.id} className="flex items-center gap-3 bg-[#0d0e1a]/40 p-2 rounded-lg hover:border-red-500/30 group cursor-pointer" onClick={() => handlePlay(anime)}>
                    <span className="text-sm font-black text-zinc-600 group-hover:text-red-500 w-5">{String(idx+1).padStart(2,'0')}</span>
            ...
            <div className="hidden md:block bg-[#0a0b12] border border-zinc-900/60 rounded-xl p-4 space-y-3">
                ...
                {scheduleItems.slice(0, 5).map(item => {
                  ...
                  return (
                    <div key={item.id} className="flex items-center gap-3 bg-[#0d0e1a]/40 p-2 rounded-lg">
                      <div className="w-10 h-10 bg-red-600 rounded flex flex-col items-center justify-center text-white shadow">
              ...
              <div className="flex gap-3 overflow-x-auto scrollbar-none pb-2">
                {!user ? (
                  <div className="text-xs text-zinc-500 py-4 w-full text-center">
                    <button onClick={() => router.push('/profile')} className="text-red-400 hover:underline">
                      Login
                  ...
                  <div className="text-xs text-zinc-500 py-4 w-full text-center flex items-center justify-center gap-2">
                    <Loader2 className="w-4 h-4 animate-spin text-red-500" /> Loading...
                  </div>
            ...
            <section className="space-y-3 bg-[#0a0b12] border border-zinc-900/60 rounded-xl p-4">
              ...
              <div className="flex gap-3 overflow-x-auto scrollbar-none pb-2">
                {publishedNews.slice(0, 3).map((n) => (
                  <div key={n.id} onClick={() => handleNewsClick(n)} className="flex-shrink-0 w-[220px] bg-[#0d0e1a]/40 border border-zinc-900/60 p-3 rounded-xl flex gap-3 cursor-pointer hover:border-red-500/30 transition-all">
        ...
        <div className="hidden lg:flex lg:w-[30%] flex-col gap-5 sticky top-20">
              ...
              <div className="flex flex-col gap-2.5">
                {watchlistItems.slice(0, 3).map((item) => (
                  <div key={item.id} className="flex items-center justify-between gap-3 group">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); toggleWatchlist(item); }} className="text-zinc-500 hover:text-red-500 shrink-0">
          ...
          <div className="bg-[#0a0b12] border border-zinc-900/60 rounded-xl p-4 space-y-3">
            ...
            <div className="flex flex-col gap-3">
              {publishedNews.slice(0, 3).map((n) => (
                <div key={n.id} onClick={() => handleNewsClick(n)} className="space-y-0.5 border-l-2 border-zinc-800 hover:border-red-500 pl-2.5 py-0.5 cursor-pointer group">
                  <h4 className="text-[11px] font-bold text-zinc-200 group-hover:text-red-400 leading-tight">{n.title}</h4>
