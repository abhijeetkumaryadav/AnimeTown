      ...
      const handler = (event: Event) => {
        ...
        if (notification?.data?.url) {
          if (navigateTo) {
            ...
            if (animeId) navigateTo('watch', undefined, { anime: animeId, ep: episodeId || '' });
          } else {
      ...
      {showPermissionBanner && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-amber-500 text-black px-4 py-3 flex items-center justify-between shadow-lg">
          <div className="flex items-center gap-2 text-xs md:text-sm font-bold">
            ...
            Enable notifications to get alerts when anime airs!
          </div>
              ...
              {DAYS.map((day, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`flex-1 min-w-[64px] md:min-w-[100px] p-2.5 rounded-xl md:rounded-2xl flex flex-col items-center justify-center border transition-all relative ${
                      isSelected
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="flex justify-between items-center px-1">
                  <h3 className="text-xs md:text-sm font-black uppercase tracking-wider text-zinc-400 flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-amber-500" /> {DAYS[activeDay]}
                  </h3>
                  {notifiedList.length > 0 && (
                    ...
                    >
                      ...
                      {showNotified ? 'All' : `Alerts (${notifiedList.length})`}
                    </button>
                  )}
                </div>
                ...
                <div className="relative pl-4 border-l border-zinc-900 space-y-4 ml-2">
                  {showSkeleton ? (
                    ...
                    <div className="text-center py-10 text-zinc-500 text-sm">
                      {showNotified ? "No alerts for this day." : "No airings scheduled."}
                    </div>
                  ) : (
                        ...
                        <div key={item.id} className="relative group flex items-start gap-3 bg-[#0b0b10] border border-zinc-900 p-3 rounded-xl md:rounded-2xl hover:border-zinc-800/80 transition-all">
                          ...
                          >
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-[9px] md:text-[10px] font-bold">
                              <span className="text-zinc-400">Ep {item.episode}</span>
                              ...
                              {isNotified && (
                                <span className="text-amber-400 bg-amber-900/30 border border-amber-800/30 px-1.5 py-0.5 rounded text-[8px] flex items-center gap-1">
                                  <BellRing className="w-2 h-2" /> Alert
                                </span>
                              )}
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            title={isNotified ? "Remove alert" : "Set alert"}
                          >
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-1">
                  ...
                  <span className="text-[10px] font-bold text-zinc-500">{allWeekItems.length}</span>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center px-1">
                ...
                <span className="text-[10px] font-bold text-zinc-500">{latestNews.length}</span>
              </div>
